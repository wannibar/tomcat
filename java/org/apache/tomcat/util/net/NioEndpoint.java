...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel,SocketChannel> {
    ...
    protected void initServerSocket() throws Exception {
        if (getUseInheritedChannel()) {
            ...
            serverSock.bind(addr, getAcceptCount()); // 初始化时进行了端口绑定, 并配置backlog参数,默认100
        }
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            ...
            if (getExecutor() == null) {
                createExecutor(); // 默认创建线程池 core10，max200，与JDK的处理有不同的地方，tomcat线程池是先到达max线程数再放入队列
            }

            initializeConnectionLatch();

            // 启动Poller线程,用于监听accept到的连接,若有就绪事件产生,则把连接事件转移到exec线程池中处理
            // Start poller thread
            poller = new Poller();
            ...
            // 启动Acceptor线程(NIO模式只有1个线程接收连接),等待客户端请求
            startAcceptorThread();
        }
    }
    ...
    protected boolean setSocketOptions(SocketChannel socket) {
        ...
        try {
            ...
            // 放入到队列中,用于Poller线程获取并处理
            poller.register(socketWrapper);
            System.out.println("poller.register(socketWrapper)添加到队列中" + socketWrapper);
            return true;
        } catch (Throwable t) {
            ExceptionUtils.handleThrowable(t);
            try {
    ...
    public class Poller implements Runnable {
        ...
        public boolean events() {
            ...
            PollerEvent pe = null;  // poll会把元素从队列中删除掉
        ...
        public void run() {
            ...
            while (true) {
                ...
                while (iterator != null && iterator.hasNext()) {
                    ...
                    // cancelledKey()
                    if (socketWrapper != null) {
                        processKey(sk, socketWrapper); // 处理事件
                    }
                }
        ...
        protected void doWrite(boolean block, ByteBuffer buffer) throws IOException {
            ...
            if (block) { //阻塞写
                ...
                do {
                    ...
                    synchronized (writeLock) {  // 写Response的具体逻辑,不断循环,直到写完
                        ...
                        if (n == 0 && (buffer.hasRemaining() || getSocket().getOutboundRemaining() > 0)) {
                            // Ensure a spurious wake-up doesn't trigger a duplicate registration
                            if (!writeBlocking) {
                                writeBlocking = true; //如果一次没写完,则通过writelock阻塞住,注册写事件,select时候notify,唤醒后继续下一次写
                                registerWriteInterest();
                            }
                            try {
                                if (timeout > 0) {
                                    startNanos = System.nanoTime();
                                    writeLock.wait(timeout); // 默认最多wait20秒
                                } else {
                                    writeLock.wait();
                                }
                            } catch (InterruptedException e) {
    ...
    protected class SocketProcessor extends SocketProcessorBase<NioChannel> {
        ...
        protected void doRun() {
            ...
            try {
                int handshake = -1;
                System.out.println("ready to process  " + event + " in SocketProcessor.run()");
                try {
                ...
                if (handshake == 0) {
                    ...
                    // getHandler() 就是ConnectionHandler
                    if (event == null) {
                        System.out.println("handshake == 0 && event == null ready to process OPEN_READ");
                    ...
                    System.out.println("handshake SelectionKey.OP_READ 继续处理");
                    socketWrapper.registerReadInterest(); // 继续读
                } else if (handshake == SelectionKey.OP_WRITE){
                    System.out.println("handshake SelectionKey.OP_WRITE 继续处理");
